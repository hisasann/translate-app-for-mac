...
test.describe('翻訳アプリ Electronテスト', () => {
  ...
  test('日本語から英語への翻訳テスト', async () => {
    ...
    // 言語ラベルが自動で切り替わることを確認
    const inputLabel = await window.textContent('.input-section .language-selector label');
    const outputLabel = await window.textContent('.output-section .language-selector label');
  ...
  test('UI要素の基本的な動作確認', async () => {
    ...
    // プレースホルダーテキストを確認
    const placeholder = await window.getAttribute('#englishText', 'placeholder');
